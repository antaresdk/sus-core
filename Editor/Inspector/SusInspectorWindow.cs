...
namespace Sharq.Core.Editor.Inspector
{
    ...
    public sealed class SusInspectorWindow : EditorWindow
    {
        ...
        void DrawOverview()
        {
            ...
            if (GUILayout.Button("BasicKit scene", GUILayout.Height(24)))
                TryOpenBasicKit();
        ...
        static void TryOpenBasicKit()
        {
            if (EditorApplication.ExecuteMenuItem("Window/SUS/Kit/Open BasicKit Scene"))
                return;

            var guids = AssetDatabase.FindAssets("BasicKit t:Scene");
            foreach (var g in guids)
            {
                var path = AssetDatabase.GUIDToAssetPath(g);
                if (path.Replace('\\', '/').Contains("BasicKit") && path.EndsWith(".unity"))
                {
            ...
            EditorUtility.DisplayDialog("BasicKit",
                "BasicKit.unity not found and DownstreamLib menu is unavailable.\n" +
                "Install com.sharq-it.sus.kit, then use Window → DownstreamLib → Open BasicKit Scene.",
                "OK");
        }
        ...
        void DrawSettings()
        {
            ...
            EditorGUILayout.Space(8);
            EditorGUILayout.HelpBox(
                "Docs: docs/SUS_INSPECTOR_PANEL_DESIGN.md · sus-core/Docs/09-compilation.md · downstream library/docs/11-devtools.md",
                MessageType.None);
        }
