    ...
    public sealed class SusSetupWizard : EditorWindow
    {
        ...
        private void RunSetup()
        {
            try
            {
                ...
                // 2b. Customization scaffold — optional AppButton.sharq is not included by default.
                string customizationMsg = "";
                if (_createCustomization)
                    customizationMsg = CreateCustomizationScaffold(root, kitAvailable: false);
        ...
        /// </summary>
        private string CreateCustomizationScaffold(string projectRoot, bool kitAvailable)
        {
            ...
            WriteIfMissing(Path.Combine(custAbs, "README.md"),
                BuildCustomizationReadme(kitAvailable));
            ...
            // Custom component: only when an optional downstream UI package is present.
            if (kitAvailable)
                WriteIfMissing(Path.Combine(custAbs, "Components", "AppButton.sharq"),
                    AppButtonTemplate);
            ...
            return $"• Customization: {custRel}/ (README, branding.uss, AppFonts.asset, AppIcons" +
                   (kitAvailable ? ", AppButton.sharq" : "") + ")\n";
        }

        // ─── Templates ───────────────────────────────────────────────────

        private static string BuildStarter(string className, bool withExample, bool withCustomization)
        {
            string chain;
            if (withCustomization)
            {
                chain =
"        // Each Use* line below is wired to a file in Customization/ — edit the file,\n" +
"        // press Play, see the change. Full map: Customization/README.md.\n" +
"        // Fully-qualified: projects with a legacy global SusApp (e.g. CompiledUI) would otherwise win name lookup.\n" +
"        Sharq.Core.SusApp.Create(GetComponent<UIDocument>())\n" +
"            .UseTheme(SusTheme.Dark)\n" +
"            // Colors & sizes \u2192 Customization/Theme/Resources/branding.uss\n" +
"            .UseCustomStyles(\"branding\")\n" +
"            // Typefaces \u2192 Customization/Fonts/AppFonts.asset (assigned in the Inspector)\n" +
"            .UseFonts(_fonts)\n" +
"            // Project icons \u2192 Customization/Icons/Resources/SusRuntime/Icons/app/\n" +
"            .UseIcons(new ResourcesFolderIconProvider(\"app\"))\n" +
(withExample
? "            .Mount<HomeScreen>();\n"
: "            // TODO: create a *.sharq screen, then mount it: .Mount<YourScreen>();\n" +
  "            .Run();\n");
            }
            else
            {
            ...
            var fontsField = withCustomization
? "    // Font set for the whole app \u2014 Customization/Fonts/AppFonts.asset\n" +
  "    // (auto-assigned by the Setup Wizard; leave empty to keep the built-in font).\n" +
...
"\n" +
"/// <summary>\n" +
"/// SUS application entry point (generated by Tools \u2192 SUS \u2192 Setup Project).\n" +
...
"    {\n" +
"        // Fluent bootstrap applies the panel TSS, loads the token cascade,\n" +
"        // mounts the root screen, then applies the theme \u2014 in the correct order.\n" +
chain +
"\n" +
"        // ── Multi-screen navigation (install the sus-router package, then use): ──\n" +
"        // Sharq.Core.SusApp.Create(GetComponent<UIDocument>())\n" +
"        //     .UseTheme(SusTheme.Dark)\n" +
"        //     .UseRouter(new Sharq.Router.SusRouter(), routes => routes\n" +
...
"}\n";
        ...
        // Root/self-target styles use inline style="" — Sharq scoping rewrites
        // ".home-screen" to ".home-screen .home-screen", which never matches the root.
        private static string BuildHomeScreen(bool withAppButton)
        {
            var appButton = withAppButton
? @"
        <sus:AppButton class=""home-screen__cta"" Label=""Get started""
                       style=""margin-top: 20px;"" />
"
: "";
            return HomeScreenTemplate.Replace("<!--APP_BUTTON-->", appButton);
        }

        private const string HomeScreenTemplate =
@"<template>
<ui:VisualElement $MainElement class=""home-screen""
                  style=""flex-grow: 1; align-items: center; justify-content: center;
                         background-color: var(--sus-bg-page);"">

    <ui:VisualElement class=""home-screen__card""
                      style=""align-items: center; padding: 32px 40px; border-radius: 16px;
                             background-color: var(--sus-bg-surface);
                             border-width: 1px; border-color: var(--sus-border);"">

        <ui:Label class=""home-screen__title"" :text=""Title.Value""
                  style=""font-size: 28px; color: var(--sus-text-primary);
                         -unity-text-align: middle-center;"" />

        <ui:Label class=""home-screen__subtitle""
                  text=""Edit HomeScreen.sharq to build your first screen.""
                  style=""margin-top: 8px; font-size: 15px; color: var(--sus-text-secondary);
                         -unity-text-align: middle-center;"" />

        <ui:VisualElement class=""home-screen__badge""
                          style=""margin-top: 20px; padding: 6px 16px; border-radius: 20px;
                                 background-color: var(--sus-primary);"">
            <ui:Label text=""SUS""
                      style=""font-size: 13px; color: var(--sus-text-on-primary);
                             -unity-text-align: middle-center;"" />
        </ui:VisualElement>
<!--APP_BUTTON-->
    </ui:VisualElement>
...
<style>
/* Root/self-target styles live in inline style="""" above (Sharq scoping would rewrite
   .home-screen to "".home-screen .home-screen"" and never match the root element).
   Child-only selectors are safe here, e.g.: */
...
";
        ...
        // Composition pattern: a thin domain wrapper over a downstream button component.
        private const string AppButtonTemplate =
@"<template>
<sus:SusButton $MainElement
               class=""app-button""
               :Text=""Label.Value""
               Variant=""elevated""
               Color=""primary""
...
// Your public API — a thin wrapper over a downstream button component.
// Usage: <sus:AppButton Label=""Save"" /> or new AppButton { Label = { Value = ""Save"" } }.
[CreateProperty] public Prop<string> Label = new(""Button"");
...
<style>
/* Colors/radius/fonts come from tokens — override the --sk- and --thm-
   variables in Theme/Resources/branding.uss to restyle ALL buttons at once.
   Put only component-specific tweaks here. */
.app-button {
    min-width: 160px;
}
</style>
...
";
        ...
        private static string BuildCustomizationReadme(bool kitAvailable)
        {
            var kitRow = kitAvailable
? "| Your component | `Components/AppButton.sharq` | used in `HomeScreen.sharq` as `<sus:AppButton>` |\n"
: "| Your component | *(optional downstream UI package)* | add `.sharq` components under `Components/` and reference them in your screens |\n";
...
"- **Your own components** - `.sharq` files anywhere UI-root'and are compiled\n" +
